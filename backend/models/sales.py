...
class SalesOrder(db.Model, TimestampMixin):
    ...
    status = db.Column(db.String(20), nullable=False, default='draft')  # draft, confirmed, shipped, delivered, cancelled
...
class SalesOrderItem(db.Model, TimestampMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint('discount_percent >= 0 AND discount_percent <= 100', name='check_so_item_discount_valid'),
    )
...
class Invoice(db.Model, TimestampMixin):
    ...
    def update_balance(self):
        ...
        self.paid_amount = sum(payment.amount for payment in self.payments if payment.status == 'completed')
